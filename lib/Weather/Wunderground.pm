...
sub _twcFormattedHash {
	...
	my $twcHash = {
		'head' => {
			...
			'ud' => $self->{UNITS} eq "m" ? "KM" : "MI"
		},
		'cc' => {
			'tmp' => $currentHash->{$self->{UNITS} eq "m" ? "temp_c" : "temp_f"},
			'wind' => {
				's' => $currentHash->{wind_mph},
				't' => $windDir,
				'gust' => ref($currentHash->{wind_gust_mph}) eq "HASH" ? "N/A" : $currentHash->{wind_gust_mph}
			},
			'vis' => $currentHash->{$self->{UNITS} eq "m" ? "visibility_km" : "visibility_mi"}
		},
		'dayf' => {
			'day' => $dailyForecasts
		}
