...
sub setScreensaverWeatherTimeMode() {
	...
	$forecastGX{$client}[$scrollIndex{$client}] = $hashDisp{$client}; # was: getFramebuf
	$forecast{$client}=();
...
sub screensaverWeatherTimelines {
	my $client = shift;
	my ($line1, $line2, $line3, $center1, $center2, $center3, $overlay1, $overlay2, $overlay3);
	my $weatherinfo;
	my $timeinfo;
	...
	if($xmax{$client} && $ymax{$client}) {
		$line1 .= $forecastGX{$client}[$scrollIndex{$client}]->{'line1'};
		$line2 .= $forecastGX{$client}[$scrollIndex{$client}]->{'line2'};
		$line3 .= $forecastGX{$client}[$scrollIndex{$client}]->{'line3'};
		$overlay2 .= $weatherinfo | $timeinfo;
	}
	else {
...
# CURRENTY UNUSED!?
sub drawForecast {
	my ($client, $icon, $text1, $text2, $text3) = @_;

	my $display = $client->display();

	$hashDisp{$client}->{'line1'} = $display->symbols('tight') 
		. $display->symbols('font') . 'weatherSB2.1' . $display->symbols('/font') . chr(0) . chr(1 + $icon) 
		. $display->symbols('/tight')
		. $display->symbols('font') . 'threeline.1' . $display->symbols('/font') . $text1 
		. $display->symbols('defaultfont');

	$hashDisp{$client}->{'line2'} = $display->symbols('font') . 'threeline.2' . $display->symbols('/font') . $text2 . $display->symbols('defaultfont');
	$hashDisp{$client}->{'line3'} = $display->symbols('font') . 'threeline.3' . $display->symbols('/font') . $text3 . $display->symbols('defaultfont');
}

sub clearCanvas {
	my $client = shift;
	$hashDisp{$client} = undef;
}

sub drawIcon {
	my $client = shift;
	my $xpos = shift;  # PARAM UNUSED
	my $ypos = shift;  # PARAM UNUSED
	my $icon = shift;

	my $display = $client->display();

	$hashDisp{$client}->{'line1'} = $display->symbols('tight') . $display->symbols('font')
		. 'weatherSB2.1' . $display->symbols('/font') 
		. chr(1 + $icon) . $display->symbols('defaultfont') . $display->symbols('/tight');

	$hashDisp{$client}->{'line2'} = $display->symbols('tight') . $display->symbols('font')
		. 'weatherSB2.1' . $display->symbols('/font') 
		. chr(0) . $display->symbols('defaultfont') . $display->symbols('/tight');

	$hashDisp{$client}->{'line3'} = $display->symbols('tight') . $display->symbols('font')
		. 'weatherSB2.1' . $display->symbols('/font') 
		. chr(0) . $display->symbols('defaultfont') . $display->symbols('/tight');
}
...
sub drawText {
	...
	$hashDisp{$client}->{'line1'} .= $display->symbols('font') . 'threeline.1' . $display->symbols('/font') . $text1 . $display->symbols('defaultfont');
	$hashDisp{$client}->{'line2'} .= $display->symbols('font') . 'threeline.2' . $display->symbols('/font') . $text2 . $display->symbols('defaultfont');
	$hashDisp{$client}->{'line3'} .= $display->symbols('font') . 'threeline.3' . $display->symbols('/font') . $text3 . $display->symbols('defaultfont');
}
